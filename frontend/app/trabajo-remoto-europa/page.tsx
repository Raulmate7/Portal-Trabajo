...
const countryKey = "europa";
const countryName = "Europa";
const countryNameEn = "Europe";
const sqlFilter = "location ILIKE $1 OR location ILIKE $2 OR location ILIKE $3";
const sqlParams = ["%europe%", "%europa%", "%eu%"];
...
export default async function RemoteEuropePage({ searchParams }: Props) {
  const resolvedSearchParams = await searchParams;
  const page = typeof resolvedSearchParams.page === 'string' ? parseInt(resolvedSearchParams.page, 10) : 1;
  const lang = resolvedSearchParams.lang === 'en' ? 'en' : 'es';
