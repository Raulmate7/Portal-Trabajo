import os
import psycopg2
from dotenv import load_dotenv

def migrate_metrics_columns():
    load_dotenv()
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        print("❌ DATABASE_URL no encontrada en las variables de entorno.")
        return

    try:
        print("🔗 Conectando a la base de datos...")
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()

        print("🛠️ Agregando columnas 'impressions_count' y 'clicks_count' a 'jobs' si no existen...")
        
        cursor.execute("""
            ALTER TABLE jobs 
            ADD COLUMN IF NOT EXISTS impressions_count INT DEFAULT 0,
            ADD COLUMN IF NOT EXISTS clicks_count INT DEFAULT 0;
        """)
        
        conn.commit()
        
        # Verificar
        cursor.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name = 'jobs' 
              AND column_name IN ('impressions_count', 'clicks_count');
        """)
        columns = [row[0] for row in cursor.fetchall()]
        print(f"📊 Columnas verificadas/creadas: {columns}")
        
        cursor.close()
        conn.close()
        print("🎉 Migración de columnas de métricas completada con éxito.")

    except Exception as e:
        print(f"❌ Error durante la migración: {e}")

if __name__ == "__main__":
    migrate_metrics_columns()
